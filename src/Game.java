...
public class Game extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            Rectangle enemy = new Rectangle(spawn.nextInt(numTilesHoriz * 10), spawn.nextInt(numTilesVert * 10), 10, 10);
            enemies.add(enemy);
        ...
        AnimationTimer timer = new AnimationTimer() {
            ...
            public void handle(long now) {
                ...
                for (Rectangle enemy : enemies) {
                    ...
                    double direction = Math.atan2(player.getY() - enemy.getY(), player.getX() - enemy.getX());
                    double enemyDeltaX = Math.cos(direction);
                    double enemyDeltaY = Math.sin(direction);

                    enemy.setX(clampRange(enemy.getX() + enemyDeltaX * 0.1, 0, pane.getWidth() - player.getWidth()));
                    enemy.setY(clampRange(enemy.getY() + enemyDeltaY * 0.1, 0, pane.getHeight() - player.getHeight()));
                }
                lastUpdate = now ;
            }
        };
